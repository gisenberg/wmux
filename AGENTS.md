...
- `wmux-hooks install prime-agent` writes an auto-loaded managed extension under `~/.prime/agent/extensions`. It reports top-level prompts as running until `agent_end`, then as awaiting input, using only capability-gated current-client attachment metadata from a compatible Prime Agent build; missing or ambiguous workspace/tab/pane identity fails closed instead of falling back to persistent worker environment. It does not turn Prime Agent into a `wmuxctl delegate` runtime and preserves an unmanaged extension at the managed path.
