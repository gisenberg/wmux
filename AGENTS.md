...
- `wmux-hooks install prime-agent` writes an auto-loaded managed extension under `~/.prime/agent/extensions`. It reports top-level prompt start/completion lifecycle without turning Prime Agent into a `wmuxctl delegate` runtime, and preserves an unmanaged extension at the managed path.
