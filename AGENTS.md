...
- `wmux-hooks install prime-agent` writes an auto-loaded managed extension under `~/.prime/agent/extensions`. It reports top-level prompt start/completion lifecycle and preserves an unmanaged extension at the managed path. Prime Agent is also a native `wmuxctl delegate` and `tui` runtime on POSIX targets; one-shot results come from its documented JSON event stream.
...
- Prime Agent one-shot delegation uses its native JSON event stream, but Prime Agent exposes neither an enforceable read-only sandbox nor a configurable approval mode. It therefore requires explicit write access, rejects unattended-mode configuration, remains POSIX-only, and is not supported by durable `--session` turns.
