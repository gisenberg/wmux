...
- `SessionBackend` is the pane execution contract.
  Keep raw PTY, durable tmux/screen, and native session-agent behavior behind the adapters in `src/server/backends/`, and extend the shared conformance suite when changing backend semantics.
  The live multiplexer startup report, not a configured preference, determines restart durability and refresh/cwd capabilities.
  The historical `windows-agent` persisted backend discriminator remains unchanged for endpoint/checkpoint compatibility; `SessionAgentBackend` is the platform-neutral execution adapter.
...
- The empty-workspace view is a DOM-backed console session launcher with explicit host selection, session search, and machine management.
  Keep it idle without animation loops or GPU rendering.
