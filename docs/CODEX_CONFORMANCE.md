...
use the attested running-server executable through the existing guard's version
and readiness checks; inode proof survives an unlinked package path, startup
errors remain visible, and optional update prompts are suppressed per process.
No native source, configuration, guard policy or service lifecycle changed.
External full check: 1,210 passed / eight skips. Isolated and deployed real
disposable-task tests passed exact history, naming, user provenance and reuse
checks. Existing panes and native service processes survived deployment.
Direct user acceptance remains pending; phone UAT remains deferred. See the
[qualification ledger](CODEX_M6_UAT.md#cli-package-update-launch-correction--2026-09-23).
