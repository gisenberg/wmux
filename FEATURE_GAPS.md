...
15. Machine pixel streams are split between the legacy helper path and an early Moonlight gateway path.
   ...
   `wmux-moonlight-gateway` is now available as an alternate stream provider that proxies a browser-native Moonlight bridge and reports gateway health to wmux. It can automate the supported pairing path by generating the Moonlight Web PIN and submitting it to Sunshine's `/api/pin`, and the Windows setup helper can install/configure/start Sunshine. It still does not implement the Moonlight/GameStream protocol, app launch, codec negotiation, stream settings, or input transport itself. The current boundary is intentional because Moonlight Web Stream is GPL-3.0-or-later and large enough that wmux should treat it as an external process unless the project deliberately accepts a license and maintenance change. A complete per-platform wmux agent should manage permissions, richer reconnect/status reporting, native display APIs, and first-class Sunshine/Moonlight lifecycle control.
