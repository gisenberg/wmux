...
test("Prime Agent installer writes an idempotent managed extension and preserves unmanaged files", { skip: process.platform === "win32" }, async () => {
  ...
  try {
    ...
    assert.match(extension, /message_start/);
    assert.match(extension, /heartbeat_prompt/);
...
test("Prime Agent extension binds each session to its forwarded pane environment", { skip: process.platform === "win32", concurrency: false }, async () => {
  ...
  try {
    ...
    // Prime heartbeat prompts begin as ordinary prepared turns, then expose their
    // distinct mode on the custom message without creating another lifecycle.
    await one.get("before_agent_start")?.({ prompt: "check the merge" }, context(0, false, "root-heartbeat"));
    const heartbeatRunId = captured.at(-1)?.runId;
    await one.get("message_start")?.({ message: {
      role: "custom", customType: "heartbeat_prompt",
    } }, context(0, false, "root-heartbeat"));
    assert.equal(captured.at(-1)?.status, "heartbeat");
    assert.equal(captured.at(-1)?.summary, "prime-agent heartbeat");
    assert.equal(captured.at(-1)?.runId, heartbeatRunId);
    const heartbeatCount = captured.length;
    await one.get("message_start")?.({ message: {
      role: "custom", customType: "agent_session_message",
    } }, context(0, false, "root-heartbeat"));
    assert.equal(captured.length, heartbeatCount);
    await one.get("agent_end")?.({ messages: [{
