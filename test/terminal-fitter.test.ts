...
test("sequenced geometry keeps the resize owner on the size the remote side applied", () => {
  const globals = globalThis as typeof globalThis & {
    ResizeObserver?: typeof ResizeObserver;
    window?: Window & typeof globalThis;
  };
  ...
  const fitter = createTerminalFitter(
    terminal,
    element,
    (dimensions) => proposals.push(dimensions),
    (cols, rows) => {
      terminal.cols = cols;
      terminal.rows = rows;
      resizes.push([cols, rows]);
  ...
  try {
    fitter.setSequencedGeometry(true);
    fitter.setForeground(true);
    fitter.setAuthoritativeSize(80, 24, true);
    fitter.fit();
    // The owner still proposes its container size to the server...
    assert.deepEqual(proposals, [{ cols: 100, rows: 30 }]);
    // ...but renders the applied grid until the server confirms the change.
    assert.deepEqual(resizes, []);
    fitter.setAuthoritativeSize(100, 30, true);
    assert.deepEqual(resizes, [[100, 30]]);

    fitter.setSequencedGeometry(false);
    Object.defineProperty(element, "clientWidth", { configurable: true, value: 1_200 });
    fitter.fit();
    assert.deepEqual(resizes.at(-1), [120, 30]);
  } finally {
    fitter.dispose();
    if (previousWindow === undefined) delete globals.window;
    else globals.window = previousWindow;
