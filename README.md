...
The managed Windows agent config uses `backend: "auto"`, preferring `pywinpty` with its native ConPTY backend so pane input, resize, rich line editing, and full-screen terminal applications go through Windows' pseudo console API instead of redirected PowerShell stdio. If pywinpty is unavailable, `auto` falls back to stdio and normalizes bare line feeds to terminal-style CRLF so prompts do not drift across the browser terminal. Explicit `backend: "conpty"` and `backend: "stdio"` values enforce ConPTY availability or select the debug fallback. Each pane process is assigned to an agent-held Windows Job Object with kill-on-close enabled, so explicitly closing a pane also terminates descendants that detached from their original shell. The job remains alive when the wmux server merely detaches, preserving sessions across service restarts. The agent also answers fixed device-attribute and operating-status queries beside ConPTY to avoid delayed browser round trips being echoed into PSReadLine; browser terminal replies are tagged so locally answered duplicates can be discarded, while cursor-position replies still come from the renderer that owns cursor state.
