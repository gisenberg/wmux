...
def runtime_label(runtime: str) -> str:
    return "Prime Agent" if runtime == "prime-agent" else runtime.capitalize()
...
def cmd_delegate(client: WmuxClient, args: argparse.Namespace) -> int:
    ...
    if args.session:
        ...
        for key in ("ready_timeout", "gate_timeout"):
            ...
            if not math.isfinite(value) or value <= 0:
                raise SystemExit(f"wmuxctl: --{key.replace('_', '-')} must be positive and finite")
    if args.runtime in {"opencode", "prime-agent"} and not args.write_access:
        raise SystemExit(
            f"wmuxctl: {runtime_label(args.runtime)} delegation cannot enforce read-only mode; "
            "add --write-access explicitly"
        )
    if args.runtime == "prime-agent" and args.unattended:
        raise SystemExit("wmuxctl: Prime Agent does not expose a configurable approval mode; omit --unattended")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    delegate = subparsers.add_parser(
        "delegate",
        help="run a visible OpenCode, Codex, Claude, or Prime Agent task",
    )
    delegate.add_argument(
        "runtime",
        choices=("opencode", "codex", "claude", "prime-agent"),
        help="agent CLI to run in the target pane",
    )
    ...
    tui = subparsers.add_parser(
        "tui",
        help="start a visible interactive OpenCode, Codex, Claude, or Prime Agent TUI on a POSIX target",
    )
    tui.add_argument(
        "runtime",
        choices=("opencode", "codex", "claude", "prime-agent"),
        help="agent CLI to start",
    )
